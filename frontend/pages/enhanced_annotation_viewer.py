...
def show_enhanced_annotation_viewer():
    ...
    st.header("🎨 Enhanced Annotation Viewer")
    st.markdown(
        """
    ...
    try:
        ...
        st.info(
            f"**Selected**: {selected_image['filename']} | **Size**: {selected_image.get('width', 'Unknown')}×{selected_image.get('height', 'Unknown')}px"
        )
    ...
    try:
        ...
        with source_col1:
            load_manual = st.checkbox("📝 Manual Annotations", value=True)
            if load_manual:
                try:
                    manual_annotations = api_client.get_annotations(selected_image_id)
                    st.success(f"✅ {len(manual_annotations)} manual annotations")
                except Exception as e:
                    st.warning(f"⚠️ Could not load manual annotations: {str(e)}")

        with source_col2:
            load_ai = st.checkbox("🤖 AI Predictions", value=True)
            if load_ai:
                try:
                    ...
                    if predictions_data and "predictions" in predictions_data:
                        ...
                        st.success(f"✅ {len(ai_predictions)} AI predictions")
                    else:
                        st.info("ℹ️ No AI predictions available")
                except Exception as e:
                    st.warning(f"⚠️ Could not load AI predictions: {str(e)}")

        with source_col3:
            load_draft = st.checkbox("✏️ Draft Annotations", value=False)
            if load_draft:
                ...
                st.info(f"ℹ️ {len(draft_annotations)} draft annotations (demo data)")

        with source_col4:
            load_imported = st.checkbox("📥 Imported Annotations", value=False)
            if load_imported:
                ...
                if uploaded_file is not None:
                    try:
                        ...
                        if isinstance(imported_data, list):
                            ...
                            st.success(
                                f"✅ {len(imported_annotations)} imported annotations"
                            )
                        else:
                            st.error(
                                "❌ Invalid JSON format. Expected array of annotations."
                            )
        ...
        if total_annotations == 0:
            st.warning(
                "⚠️ No annotations available for this image. Please select at least one annotation source or upload annotations."
            )

            # Show sample data option
            if st.button("🧪 Load Sample Annotations (for testing)"):
            ...
            if "sample_manual" in st.session_state:
                ...
                st.info("ℹ️ Using sample annotation data for demonstration")
                ...
                if manual_annotations and ai_predictions:
                    ...
                    if overlaps:
                        ...
                        for i, overlap in enumerate(overlaps[:5]):  # Show first 5
                            st.write(
                                f"• Manual #{overlap['manual_idx']} ↔ AI #{overlap['ai_idx']} (IoU: {overlap['iou']:.3f})"
                            )
                    else:
                        st.info(
